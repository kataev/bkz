$(function () {
    json_parse = function (json) {
        ...
        firing.filter(function (e, i, a) {
            return e.position && data[e.field]
        })
        ...
        firing.forEach(function (e, index, array) {
            ...
            $('<td>').text((e.target[number] - this[e.field]).toFixed(1)).appendTo(tr4)
        }, data)

        var $chart = $('#firing').empty()
        ...
        var y = d3.scale.linear()
            .range([height, 0])
            .domain(d3.extent(_.union(_.flatten(_.map(firing, function (e) {
                return e.target
            })),
                firing.filter(function (e, i, a) {
                    return e.position && data[e.field]
        ...
        var points = d3.svg.axis()
            ...
            .orient("bottom")
            .ticks(2)
            .tickValues(firing.map(function (e, i) {
                return e.position
            }))
        ...
        var line = d3.svg.line()
            // .interpolate("basis")
            .x(function (d) {
                return positions(d.position || null)
            })
            .y(function (d) {
                return y(data[d.field])
            })
            .defined(function (d) {
                return (data[d.field])
            })
        ...
        var target = d3.svg.line()
            ...
            .y(function (d) {
                console.log(number, d.target[number]);
                return y(d.target[number]);
            })
