...
function transfer(data,param,node) {
    ...
    var rect = mark.selectAll("rect.mark")
        ...
        .attr("width", z)
        .attr("height", z)
        .attr("x", function (d) { return z * marks[d.brick_from__mark]})
        .attr("y", function (d) { return z * marks[d.brick__mark]})
        .attr("class", function (d) { return "mark q" + color(d.amount__sum).toFixed() + "-9"; })
        .attr("transform", "translate(10,10)")
        .append('title')
        .text(function (d) { return 'Марка:' + d.brick_from__mark + ' > ' + d.brick__mark + ' Кол-во:' + d.amount__sum })
        .datum(function (d) { return d.amount__sum })
}
...
function total(data,param,node){
    ...
    var weight_label = svg.selectAll("text")
        ...
        .enter().append('text')
        .attr('x',function(d){return d*color_c.length * z + color_c.length * z/2+10})
        .attr('y',20)
    ...
    var rect = values.selectAll("rect.brick")
        ...
        .attr("width", z)
        .attr("height", z)
        .attr("x", function (d) { return color_c.length * z * mass_c[d.brick__weight] + d.brick__color * z })
