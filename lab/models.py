...
validate_slash_separated_float_list = RegexValidator(slash_separated_float_list_re,
                                                     u'Вводите числа разеделённые дробью', 'invalid')
...
class Bar(models.Model, ShiftMixin, UrlMixin):
    ...
    def __unicode__(self):
        if self.pk:
            return u'Брус от %s с телеги №%s' % (ru_date(self.datetime), self.tts)
        else:
            return u'Новый брус'
...
class Raw(models.Model, ShiftMixin, UrlMixin):
    ...
    def __unicode__(self):
        if self.pk:
            return u'Сырец от %s с телеги №%s' % (ru_date(self.datetime), self.tts)
        else:
            return u'Новый сырец'
...
class Half(models.Model, ShiftMixin, UrlMixin):
    ...
    cause = models.ManyToManyField('lab.Cause', verbose_name=u'<attr title="Дефекты">Деф</attr>', null=True, blank=True,
                                   limit_choices_to={'type': 'half'})
    ...
    def __unicode__(self):
        if self.pk:
            return u'Полуфабрикат от %s с поз №%d, путь №%d, ТТС № %s' % (
            ru_date(self.datetime), self.position, self.path, self.tts)
        else:
            return u'Новый полуфабрикат'
...
class WaterAbsorption(models.Model, UrlMixin):
    ...
    def __unicode__(self):
        if self.pk:
            return u'%.2f от %s для %s %s' % (
            self.value, ru_date(self.date), self.get_width_display(), self.get_color_display())
        else:
            return u'Новая проба водопоглащения'
...
class SEONR(models.Model, UrlMixin):
    ...
    def __unicode__(self):
        if self.pk:
            return u'%s для %s' % (self.get_name, self.get_color_display())
        else:
            return u'Новое значение радионуклидов'
...
class Batch(UrlMixin, models.Model):
    ...
    def __unicode__(self):
        if self.pk:
            return u'Партия № %d, %s' % (self.number, ru_date(self.date))
        else:
...
class Part(models.Model):
    ...
    def __unicode__(self):
        if self.pk:
            ...
            return u'Новый выход с производства'
