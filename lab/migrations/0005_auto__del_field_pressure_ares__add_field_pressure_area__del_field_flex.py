...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Pressure.ares'
        db.delete_column('lab_pressure', 'ares')

        # Adding field 'Pressure.area'
        db.add_column('lab_pressure', 'area',
                      self.gf('django.db.models.fields.FloatField')(default=0),
                      keep_default=False)

        # Deleting field 'Flexion.ares'
        db.delete_column('lab_flexion', 'ares')

        # Adding field 'Flexion.area'
        db.add_column('lab_flexion', 'area',
                      self.gf('django.db.models.fields.FloatField')(default=0),
                      keep_default=False)

    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'Pressure.ares'
        raise RuntimeError("Cannot reverse this migration. 'Pressure.ares' and its values cannot be restored.")
        # Deleting field 'Pressure.area'
        db.delete_column('lab_pressure', 'area')


        # User chose to not deal with backwards NULL issues for 'Flexion.ares'
        raise RuntimeError("Cannot reverse this migration. 'Flexion.ares' and its values cannot be restored.")
        # Deleting field 'Flexion.area'
        db.delete_column('lab_flexion', 'area')

    models = {
        'lab.bar': {
            'Meta': {'object_name': 'Bar'},
            'cutter': ('django.db.models.fields.CommaSeparatedIntegerField', [], {'max_length': '3000'}),
            'datetime': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 7, 20, 14, 46, 50, 4)'}),
