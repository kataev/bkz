...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Warren.amount'
        db.delete_column('make_warren', 'amount')

        # Deleting field 'Warren.tts'
        db.delete_column('make_warren', 'tts_id')

        # Deleting field 'Warren.timestamp'
        db.delete_column('make_warren', 'timestamp')

        # Adding field 'Warren.brocken'
        db.add_column('make_warren', 'brocken',
                      self.gf('django.db.models.fields.IntegerField')(default=0),
                      keep_default=False)

        # Adding field 'Warren.forming'
        db.add_column('make_warren', 'forming',
                      self.gf('django.db.models.fields.related.ForeignKey')(blank=True, related_name='warrens', null=True, to=orm['make.Forming']),
                      keep_default=False)

        # Adding M2M table for field cause on 'Warren'
        db.create_table('make_warren_cause', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('warren', models.ForeignKey(orm['make.warren'], null=False)),
            ('cause', models.ForeignKey(orm['lab.cause'], null=False))
        ))
        db.create_unique('make_warren_cause', ['warren_id', 'cause_id'])

    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'Warren.amount'
        raise RuntimeError("Cannot reverse this migration. 'Warren.amount' and its values cannot be restored.")
        # Adding field 'Warren.tts'
        db.add_column('make_warren', 'tts',
                      self.gf('django.db.models.fields.related.ForeignKey')(related_name='warrens', null=True, to=orm['make.Forming'], blank=True),
                      keep_default=False)


        # User chose to not deal with backwards NULL issues for 'Warren.timestamp'
        raise RuntimeError("Cannot reverse this migration. 'Warren.timestamp' and its values cannot be restored.")
        # Deleting field 'Warren.brocken'
        db.delete_column('make_warren', 'brocken')

        # Deleting field 'Warren.forming'
        db.delete_column('make_warren', 'forming_id')

        # Removing M2M table for field cause on 'Warren'
        db.delete_table('make_warren_cause')
